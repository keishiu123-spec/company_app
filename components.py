                ...
                if message["content"]["mode"] == ct.ANSWER_MODE_1:
                    ...
                    if not "no_file_path_flg" in message["content"]:
                        ...
                        if "main_page_number" in message["content"]:
                            st.success(
                                utils.format_file_info(main_file_path, message["content"]["main_page_number"]),
                                icon=icon
                            )
                        else:
                            st.success(utils.format_file_info(main_file_path), icon=icon)
                        ...
                        if "sub_message" in message["content"]:
                            ...
                            for sub_choice in message["content"]["sub_choices"]:
                                ...
                                st.info(
                                    utils.format_file_info(
                                        sub_choice['source'],
                                        sub_choice.get('page_number')  # ある時だけページNo
                                    ),
                                    icon=icon
                                )
...
def display_search_llm_response(llm_response):
    ...
    if llm_response["context"] and llm_response["answer"] != ct.NO_DOC_MATCH_ANSWER:
        ...
        st.success(utils.format_file_info(main_file_path, main_page_number), icon=icon)
